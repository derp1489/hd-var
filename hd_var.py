...
def hd_var(pnl, ci=0.95):
    pnl = np.array(sorted(pnl, reverse=False))
    num_data = len(pnl)
    w = (num_data + 1) * ci
    z = (num_data + 1) * (1 - ci)
    beta_cdf = np.vectorize(partial(sp.stats.beta.cdf, a = z, b = w))
    temp_arr = np.array([1/num_data * i for i in range(num_data + 1)])
    x_arr = list(beta_cdf(temp_arr))
    # x_lst = np.array(
    #     [
...
def hd_incremental(
    ...
    increment = hd_var_annual(var_after, factor) - hd_var_annual(var_before, factor)
    return increment